...
// CONFIRMATION DIALOG
// ============================================================
function ConfirmDialog({ isOpen, onClose, onConfirm, title, message, confirmLabel = "Konfirmasi", confirmColor = "bg-[#1297DC] hover:bg-[#0e7ab8]", isDestructive = false, isLoading = false }) {
  useEffect(() => {
      ...
      >
        ...
        <div className="px-6 pb-6 flex gap-3">
          ...
          <button
            onClick={onConfirm}
            disabled={isLoading}
            className={`flex-1 px-4 py-2.5 rounded-xl text-white text-sm font-semibold transition disabled:opacity-50 flex items-center justify-center gap-2 ${isDestructive ? "bg-red-500 hover:bg-red-600" : confirmColor}`}
          >
            {isLoading ? (
              <>
...
// ============================================================
// VALIDATION ERROR POPUP (inline profesional)
// ============================================================
...
const ACTION_LABEL = {
  ...
  DELETE_COMMENT:  { label: "Hapus Komentar",   cls: "bg-orange-100 text-orange-700 border border-orange-300" },
...
function SkeletonRow() {
  return (
    <tr className="border-b border-gray-50 animate-pulse">
      <td className="px-6 py-4"><div className="h-4 bg-gray-200 rounded w-32 mb-1" /><div className="h-3 bg-gray-100 rounded w-20" /></td>
      <td className="px-4 py-4"><div className="h-5 bg-gray-200 rounded-full w-24" /></td>
      ...
      <td className="px-4 py-4"><div className="h-5 bg-gray-200 rounded-full w-24 mx-auto" /></td>
...
function ReportDetailModal({ report, onClose, onActionSuccess }) {
  ...
  // Fetch detail konten yang dilaporkan
  useEffect(() => {
    const fetchDetail = async () => {
      try {
        const token = localStorage.getItem("token");
        const res = await fetch(
          `${backendUrl}/api/report-management/${report.id}`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
        if (res.ok) {
          const data = await res.json();
          setContentDetail(data.data?.contentDetail || null);
        }
      } catch { /* ignore */ }
  ...
  const handleSuspendDaysChange = (val) => {
    ...
    setFormData(prev => ({
      ...prev,
      suspendDuration: days > 0 ? days : null,
    }));
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // 1. Mark as in review terlebih dahulu jika masih PENDING
      if (report.status === "PENDING") {
        await fetch(`${backendUrl}/api/report-management/${report.id}/review`, {
          method: "PUT",
          headers: { Authorization: `Bearer ${token}` },
        });
      }
      ...
      // 2. Ambil tindakan
      const res = await fetch(
        `${backendUrl}/api/report-management/${report.id}/action`,
        {
          method: "POST",
          headers: {
      ...
      toast("Tindakan berhasil diambil dan laporan telah diperbarui.", "success");
      onActionSuccess?.();
      handleClose();
    } catch (err) {
      toast("Gagal mengambil tindakan: " + err.message, "error");
    } finally {
  ...
  const isDestructive = ["BAN_PERMANENT", "DELETE_CONTENT", "DELETE_COMMENT", "SUSPEND_ACCOUNT"].includes(formData.actionTaken);
  ...
  return (
    <>
          ...
          <div className="px-6 py-5 space-y-5 max-h-[70vh] overflow-y-auto">
            ...
            {loadingDetail ? (
              ...
              <div className="bg-red-50 border border-red-100 rounded-xl p-4">
                ...
                <p className="text-sm font-semibold text-gray-900">
                  {contentDetail.title || contentDetail.series?.title || contentDetail.ebooks?.title ||
                    contentDetail.comics?.title || contentDetail.podcasts?.title || "—"}
                </p>
                <p className="text-xs text-gray-500 mt-1">
                  Tipe: <strong>{report.contentType?.replace(/_/g, " ")}</strong>
                </p>
              </div>
            ) : (
            ...
            <div>
              ...
              <p className="text-sm text-gray-800 font-medium">
                {report.isAnonymous ? "Anonim" : (report.email || "Tidak diketahui")}
              </p>
            </div>
      ...
      <ConfirmDialog
        ...
        title={isDestructive ? "Konfirmasi Tindakan" : "Konfirmasi Tindakan"}
        message={
          formData.actionTaken === "BAN_PERMANENT"
            ? `Anda akan melakukan ban permanen terhadap akun ini. Tindakan ini tidak dapat dibatalkan.`
            : formData.actionTaken === "SUSPEND_ACCOUNT"
            ? `Anda akan suspend akun ini selama ${formData.suspendDuration} hari. Apakah Anda yakin?`
            : formData.actionTaken === "DELETE_CONTENT"
...
export default function ContentReportsPage() {
  ...
  const fetchReports = useCallback(async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(
        `${backendUrl}/management/content/${contentId}/reports?type=${contentType}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      if (!res.ok) throw new Error("Gagal mengambil data laporan konten");
      const data = await res.json();
      setReports(data.data || []);
    } catch (err) {
      setError(err.message);
      toast("Gagal memuat data laporan. Silakan coba lagi.", "error");
    } finally {
      setLoading(false);
    }
    ...
    <div className="min-h-screen bg-[#F5F5F5] p-6">
      ...
      <div className="mb-6">
        <div className="flex items-center gap-1.5 text-sm text-gray-400 mb-3">
          ...
          <span className="text-gray-600 font-medium">Laporan Konten</span>
        </div>
                ...
                <tr>
                  <td colSpan={7} className="py-20 text-center">
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <p className="text-gray-400 text-sm font-medium">Tidak ada laporan untuk konten ini</p>
                    </div>
                  </td>
                </tr>
              ) : (
                  ...
                  <tr key={report.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          {report.isAnonymous && (
                            <p className="text-xs text-gray-400">Laporan anonim</p>
                          )}
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-4 max-w-[220px]">
                      <p className="text-sm text-gray-600 line-clamp-2 leading-relaxed">
                        {report.reportDetail || <span className="text-gray-300 italic">Tidak ada detail</span>}
                      </p>
                    </td>
